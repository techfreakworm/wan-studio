...
| 13 | wan2.2_s2v_14b | ✅ new | ✅ bf16 | ✅ | ✅ | ✅@17f | ✅ | ✅ identity+audio | ⬜ | **2026-06-22 PASS @480p (512×704)**. Upstream `wan.WanS2V` ported to MPS (NOT in diffusers): scripts/s2v_smoke.py vendors wan + shims — device cuda→mps factory (handles `\|` unions + isinstance), float64→fp32 downcast, autocast→mps bf16 + **2 fp32 islands** (time_embedding, head; MPS autocast has no fp32), flash_attention replaced in all 4 namespaces (q_scale+k_lens+chunked, MHA no-GQA), wav2vec2 on CPU @16kHz mono, bf16 VAE. 16-step UniPC: autocorr **0.903**, sharp 124, motion 8.2, all gates PASS. EYEBALL: cat-in-sunglasses, IDENTITY preserved + AUDIO-driven mouth (closed→open→partial). Mem: per-forward + pre-decode empty_cache + **offload_model=True REQUIRED** (DiT→CPU before decode); peak **~110GB REAL** (vm_stat active+wired+compressed, NOT driver) — EXCEEDS the 92.4 conservative cap but under 128 physical, completes via offload+empty_cache. ⚠️ **memcheck UNDER-COUNTS s2v decode**: s2v decodes motion-context+target (~89 pixel/~23 latent frames), not the 17 requested — the static preflight estimated ~5× too few decode frames → didn't refuse the 20-step config that OOM'd at 182GB driver. KNOWN PREFLIGHT GAP (s2v not gated by app memcheck anyway — runs as scoped subprocess w/ free_all + its own empty_cache/offload). ⚠️ **SCHEDULER: 16-step UniPC** (deviates from the project FlowMatchEuler-NOT-UniPC rule) — empirically CLEAN here (sat 0.544 natural, not neon; this is the VENDORED wan sampler, not the diffusers UniPC corrector that neoned) → conscious choice, not oversight. ⚠️ runs via MPS bf16-autocast — explicit-bf16 rewrite staged as known-good fallback. Autoregressive color drift over frames (cat→yellow/water→cyan; not banding) — euler-vs-UniPC A/B is a follow-up to confirm drift is autoregressive vs scheduler. |

Legend: ✅ done · ⏳ in progress · ⬜ todo · — not started · reuse/cached = satisfied via another row.

## Phase B — in-app Playwright sweep (Commit #2 gate)
Local app: `WAN_STUDIO_PORT=7863 python app.py`, driven via Playwright MCP.
- [x] **App loads** — all 8 mode tabs render (t2v/i2v/ti2v/flf2v/v2v/vace/s2v/animate) + Gallery/Settings; dtype banner shows bf16/bf16.
- [x] **Control: Generation 2.1↔2.2 switch** — dropdown toggles; value commits (verified Wan2.1→2.2→2.1).
- [x] **Control: Fast↔Quality preset toggle** — `ws-pill-active` moves to the clicked pill.
- [x] **s2v + animate tabs wired** — registered in HANDLER_REGISTRY (was toast-only); `_inputs_for` + `_run_s2v`(scoped subprocess)/`_run_animate`(CPU preproc + handle) added; tab inputs render correctly.
- [x] **T2V end-to-end in-UI** — Wan2.1 Fast 17f/480p: 4-step Lightning, MP4 rendered in the output `<video>`. Generic text-diffusers UI→inference→export pipeline proven.
- [x] **I2V end-to-end in-UI** — Wan2.1 17f/480p, car.png init: PASS (autocorr 0.938, sharp 577), MP4 rendered. Image-diffusers path (gr.Image→PIL) proven.
- [x] **S2V end-to-end in-UI** — uploaded image+talk.wav → Generate → `_run_s2v` free_all (app→26GB) → s2v_smoke.py SUBPROCESS (8-step) → 17f mp4 PASS, rendered in UI. The scoped-subprocess wiring proven end-to-end (hardest new path).
- [x] **Animate end-to-end in-UI** — character+driving video → CPU pose/face preproc → WanAnimatePipeline 480p/13f (20-step, ~26s/step CPU-QR fallback): PASS (autocorr 0.833, sharp 401), mp4 + driving video both rendered. Preproc-subprocess + in-process-handle wiring proven.
- [x] **V2V end-to-end in-UI** — restyle of a 17f source clip, Wan2.1 Quality (28 steps): PASS (832×480, autocorr 0.902, sharp 316), mp4 rendered. Video-decode + Quality-diffusers path proven.
- [x] **TI2V end-to-end in-UI** — car.png + prompt, 1280×704/13f (own z48 VAE): PASS (autocorr 0.899), mp4 rendered.
- [x] **VACE end-to-end in-UI** — Reference submode + ref image + prompt, 480p/17f Quality (30 steps, ~93s/step ≈ 46min): PASS (832×480, autocorr 0.938, motion 7.4), mp4 rendered.
- [x] **FLF2V end-to-end in-UI** — start+end frame + prompt, 720p/13f Quality (40 steps @720p chunked-flash, ~158s/step, ~108min): PASS (704×1296, autocorr **0.951**, motion 5.08). EYEBALL: coherent first→last morph (blue convertible on coastal-sunset road; start≈first frame, end≈last frame, identity preserved). cap+guard fix verified, no error. **8/8 tabs now fully rendered in-UI.**
- [x] **UX fix 1** — local-MPS duration default 3.4s→1.0s (53f→17f): old default ALWAYS tripped the memory guard → local UI couldn't generate out-of-box. (HF Spaces keeps 3.4s.)
